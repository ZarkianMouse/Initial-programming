...
void avg(int arr[], int sizeArr, int sum)
{
    ...
    // finds the total amount of numbers inputted
    for (int i = 0; i < sizeArr; i++)
    {
        total += arr[i];
    }

    // to be activated if user has not yet used the summation feature
    if (sum == 0)
    {
        sum = getSum(arr, sizeArr);
    }

    // activated if no numbers have yet been inputted
    if ((total == 0) || (sum == 0))
    {
        cout << "ERROR\n";
        return;
    ...
    {
        average = static_cast<float>(sum) / total;
        cout << average << endl;
        return;
    }
}
